...
import { extractUserOffer } from "../services/negotiation.service.js";
  ...
  io = new Server(httpServer, {
    cors: {
      origin: [
        "http://localhost:5173",
        "http://127.0.0.1:5500",
        "https://negotiate-x-frontend.onrender.com",
      ],
      credentials: true,
    },
  });
...
if (intent === "offer" && userOffer) {
  const counter = Math.floor((game.currentPrice + userOffer) / 2);
  newPrice = Math.max(counter, game.minPrice);
} else {
  newPrice = updatePrice(game, intent);
}
